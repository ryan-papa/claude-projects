...
| **⛔ 민감 정보 금지** | 호스트·계정명·포트·인증서 경로·내부 IP 등 인프라 정보는 **어떠한 문서에도 작성 금지**. 환경변수 참조(`$DB_HOST`)로 대체하고 실제 값은 "관리자에게 별도 문의"로 안내 |
...
**자동 전환:** 모든 단계 완료 시 다음 단계 자동 진입. 커밋·PR까지 자동. **배포[11]만 사용자 승인 대기.**
