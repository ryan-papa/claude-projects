...
- QA([8]), 코드리뷰([9]) **생략 불가**
- **[4][5][9] Codex 추가 리뷰 (Claude-led 한정)** — Claude-led 모드에서만 Claude 서브에이전트 통과 후 `Codex-led findings-first review` 1회 실행, High/Critical 지적 반영 필수. Codex-led 모드는 본 항 N/A (메인이 Codex). SSOT: [`../harness-absolute-rules.md`](../harness-absolute-rules.md) "작성 모드 및 리뷰 매트릭스"
