...
모든 DDL(신규 테이블·ALTER·마이그레이션)은 아래 규칙을 **강제**한다. 테스트(구조 검증) 가 이 규칙 준수를 확인할 수 있어야 한다.
...
- 용도(쿼리 패턴·배치명)를 적어 향후 삭제 판단이 가능하게
...
- 인덱스 추가/삭제
- 위 중 하나라도 COMMENT 미기재 시 CI 실패로 처리 (향후 규칙)
