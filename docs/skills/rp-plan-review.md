...
1. **⛔ Claude 기획 리뷰는 반드시 Agent 툴의 서브에이전트로 실행** (`subagent_type=general-purpose`). 메인 셀프 채점 **금지**. 서브에이전트 프롬프트 4 필수 항목 (a)~(d) → SSOT: [`../harness-absolute-rules.md`](../harness-absolute-rules.md) "[리뷰 단계 서브에이전트 필수]" 절 참조. 본 단계 적용값: (a) PRD 파일 경로 전체 (b) 9항목 (c)·(d) SSOT 그대로
