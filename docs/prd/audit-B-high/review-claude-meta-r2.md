...
1. **CLAUDE.md L127 "자동 머지 가드 4종 AND" 단어 사용**: contextual 한 줄(자동 전환 안내 맥락)이라 SSOT 위반은 아니지만, 향후 `**`만 풀고 평문 정의로 회귀할 위험 있음. PR-C 에서 absolute-rules SSOT 링크화 권장 (감점 없음, 강화 제안)
