...
| 모드 | 메인 런타임 | 진입 |
|------|-----------|------|
| Claude-Lead | Claude | Claude Code (`/rp-*` 슬래시 명령) |
| Codex-Lead | Codex | Codex CLI (`.codex/skills/` SKILL) |

사용자가 진입한 런타임 = 메인. 동일 변경을 추후 다른 런타임에서 다시 열어도 **기존 리뷰 대체 금지** — 추가 독립 검토로만 기록.

### 리뷰 구성

**⛔ 런타임 = 리뷰어.** 진입한 런타임의 서브에이전트만 채점한다. **교차 런타임 추가 리뷰 전면 금지** — Claude-Lead 에서 Codex 호출 금지, Codex-Lead 에서 Claude 호출 금지.

> **재평가 조건**: 교차 리뷰 제거로 이중 관점 검증을 포기했다. **[9] 통과 후 머지된 변경이 롤백·핫픽스를 유발한 사례 3건 누적 시** 교차 리뷰 재도입 여부를 재검토한다. 발생 시 원인·단계를 `docs/research/` 에 1줄 기록. 기록 없이 "리뷰가 약하다"는 인상만으로 재도입 금지.

| 모드 | 서브에이전트 (단계당 재시도 포함) | 외부 추가 리뷰 |
|------|------------------------------|-------------|
| Claude-Lead | Claude 서브에이전트 ([4]·[5] 최대 3회 / [9] 최대 5회) | **없음** — Codex 호출 금지 |
| Codex-Lead | Codex 서브에이전트 ([4]·[5] 최대 3회 / [9] 최대 5회) | **없음** — Claude 호출 금지 |
...
| [9-코드] Code review | 코드 (정확성·설계·가독성·테스트·보안·성능·유지보수성) | 메인 런타임의 서브에이전트 |
| [9-인프라] Infra review | 인프라 접점 (SQL·Redis·비동기·직렬화·배포) — **무점수 BLOCK/ASK/WARN**, 정적 분석 전용 | 메인 런타임의 서브에이전트 |

**병렬 실행**: `[4] ∥ [5]`, `[9-코드] ∥ [9-인프라]` 는 동일 메시지에서 **동시 발사**(컨텍스트 미공유 독립 판정). Codex-Lead 는 `spawn_agent` 동시 발사 불가 시 **순차 fallback 허용** — 판정 규칙·통과 조건 동일하며 **병렬 여부를 통과 조건에 넣지 않는다**.

**조건부 재검증**: 미달·차단된 축만 재실행하되, 수정이 상대 관점 영역을 건드리면 통과한 축도 1회 재검증한다. 재검증은 카운트 미소비이며 **같은 사이클 내 연쇄 재검증 금지** — 재검증이 미달이면 그 축은 다음 사이클의 미달 축으로 전환된다(종료성은 재시도 한도가 보장).

**판정 강등은 결정 이관**: 인프라 축의 BLOCK·ASK 에 대해 Dev Agent 가 정적 근거로 반박하면 한 단계 강등(BLOCK→ASK, ASK→사용자 결정)된다. 최종 심사 주체는 **사용자**이므로 이는 판정 번복이 아니라 결정 이관이며, 메인 셀프 채점 금지와 충돌하지 않는다. 사이클당 1회, 재반박 금지. 상세: [`harness-infra-review.md`](harness-infra-review.md)
...
- 모든 채점은 반드시 서브에이전트 분리 호출:
  - Claude-Lead → Agent 툴 `subagent_type=general-purpose`
  - Codex-Lead → `spawn_agent` 별도 컨텍스트 (Claude Agent 툴과 동일 hook·slash 런타임 아님)
...
| 단계 | 평가 미달 재시도 한도 | 카운터 |
|:---:|:---:|------|
| [4] Plan review · [5] Engineering review | 인-메모리 최대 **3회** | **축별 독립** |
| [9] Code review + Infra review | 인-메모리 최대 **5회** | **두 축 공통** — 소비 단위는 **사이클**(한 사이클에서 어느 축이든 1개 이상 재실행하면 1회) |

- 1회 = 서브에이전트 재호출 + PRD/코드 갱신 사이클
- 인프라 축 **ASK 발생 자체는 미소비**. ASK 후 코드 수정하여 재실행하면 그 사이클이 1회 소비. 실인프라 접근 위반으로 인한 결과 폐기·재발사도 미소비
- **한도 미달 시 자동 진행 중단 + 사용자 결정 요청** (강행 / 재설계 / 중단). 임계 완화·임의 통과 금지
- 기술적 실패(서브에이전트 오류·토큰·형식)는 별도 카운팅 — 단계 무관 **1차 시도 후 추가 최대 2회 재호출(총 최대 3회)** 후 지속 시 사용자 보고

## 문서·리뷰 절차

- **[리뷰 단계 서브에이전트 필수]** `/rp-plan-review`·`/rp-eng-review`·`/rp-code-review`·`/rp-infra-review` 적용 — 작성 모드별 서브에이전트는 SSOT [작성 모드 및 리뷰 매트릭스](#작성-모드-및-리뷰-매트릭스-ssot) 참조:
  - 리뷰 단계 진입 시 서브에이전트 호출은 단계 실행 자체다. 추가 사용자 승인·확인 질문을 만들지 않고 자동 호출한다. 사용자 결정 요청은 재시도 한도 도달·기술 실패 지속·**인프라 축 ASK 발생**·명시된 중단 조건에서만 수행
  - 서브에이전트 프롬프트 4 필수 항목(SSOT): (a) 대상 파일 경로 + 참고 파일, (b) 채점 기준(단계별 항목 수: 4=9, 5=5, 9-코드=7, **9-인프라=무점수 BLOCK/ASK/WARN 5영역**), (c) 독립 판정 지시(메인 컨텍스트 미공유), (d) 역할 경계: 본 단계 채점만 수행 — **응답 텍스트로만 결과 반환, 어떤 결과 파일도 생성 금지**. **인프라 축은 실인프라(DB·Redis·k8s·브로커·MCP DB 도구) 조회·접속 전면 금지 — 정적 분석 전용, 위반 시 결과 폐기 + 재발사(상한 2회, 3회째 중단 + 사용자 보고)** (Claude-Lead: Codex 실행 금지 / Codex-Lead: Claude 호출·결과 합성 금지) (`.codex/skills/` 어댑터 노트 동시 참조). **스킬 파일은 본 (a)~(d) 본문 중복 금지, 링크 참조만 허용**
  - Fallback: 기술적 실패 시 **1차 시도 후 추가 최대 2회 재호출(총 최대 3회)** → 지속 시 사용자 즉시 보고. 셀프 채점 우회 금지
  - 평가 미달: PRD 본문 갱신 후 새 서브에이전트 재실행 (기술 실패와 구분). 인-메모리 한도는 SSOT [재시도 한도](#재시도-한도-단계당) ([4]·[5] 3회 / [9] 5회), 회차별 별도 파일·점수 보존 없음. 한도 미달 시 동일 SSOT 적용
  - **증거 파일 보존 없음**: 리뷰 지적은 PRD 본문 자체에 반영된 결과물로만 남는다. `review-claude-*.md`·`review-codex-*.md` 어떤 형태도 생성 금지
- 기능 변경 시 코드 전에 PRD 문서 먼저 업데이트 + 리뷰
- **⛔ 교차 런타임 추가 리뷰 전면 금지**: [4]·[5]·[9] 는 진입 런타임의 서브에이전트 채점만으로 판정한다
  - Claude-Lead 에서 `codex` 명령·`/codex:*` 플러그인 호출 **금지**. Codex-Lead 에서 Claude 호출 **금지**
  - 통과 조건은 **동일 런타임 서브에이전트의 판정 단일 기준** — [4]·[5]·[9-코드]는 점수(평균 ≥8.0 + 각 항목 ≥7), [9-인프라]는 BLOCK 0건 + 미해결 ASK 0건. 두 축은 AND 결합하되 서로의 점수를 조정하지 않는다. **외부(교차 런타임) 리뷰 반영 여부를 통과 조건에 추가 금지**
  - 사용자가 하네스 게이트 **밖에서** 별도 도구로 검토하는 것은 자유 — 단, 그 결과를 단계 통과 근거로 기록 금지

## 테스트·QA·게이트

- 테스트 코드 없이 커밋/머지 절대 금지
- 테스트·빌드 미통과 시 다음 단계 진행 금지
- **QA·코드 리뷰·인프라 리뷰 단계 생략 금지 (게이트 강화)**: 세 단계 모두 완료 여부 배포 전 체크. 하나라도 미수행이면 배포 차단. "단위 테스트 통과 = QA 대체 불가". 단 인프라 축의 **정당한 skip**(콘텐츠 프로젝트·하네스 메타 변경·5영역 무해당 N/A 반환)은 이수로 간주
...
- **⛔ 리뷰 통과 후 ship 확인 질문 금지**: [9] 코드·인프라 **양축** 통과 시 `rp-ship`(커밋→PR→자동머지) **자동 진입**. "진행할까요?"·"ship 할까요?"·"신중히 멈출까요?" 등 확인 질문 금지 (하네스 메타 변경·머지 가드 규칙 자체 변경 포함 — 변경이 위험해 보여도 동일). 사용자 결정 요청은 자동 머지 가드 실패·재시도 한도 도달·fail-closed 등 **명시된 중단 조건에서만**. 비상 정지는 `RP_SHIP_MANUAL=1` 환경변수로만
...
- **하네스 메타 변경 단축 경로**: `rp-init`·`rp-specify`·`rp-task`·`rp-dev` 스킵 + feat 브랜치 + `rp-prd` 간소(변경 이유·영향 파일·롤백·검증 4섹션) + 리뷰 + `rp-ship`. 완전 생략은 금지
  ...
  - **예외**: `docs/skills/` 전용 메타 변경 (코드 동작 영향 0건 — `repositories/[project]/` 산하 파일 미수정) 은 [9] 만 적용 ([4]·[5] 스킵). 영향 파일 범위가 docs/skills/ + 동기화된 `.codex/skills/` 로 한정될 때만 적용. **[9] 2축 중 인프라 축은 메타 변경이므로 발사 skip → 코드 축만 수행**
