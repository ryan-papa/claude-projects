...
- **Codex 추가 리뷰** — 기획·엔지·코드 리뷰 3단계 모두 Claude 완료 후 `/codex:review` 포그라운드 1회 실행, High/Critical 지적 반영. Codex CLI가 토큰/기능 신호(rate/usage limit·quota·기능 미지원 등 9개 패턴)를 명시 출력하면 1회 스킵 후 진행, 그 외 비정상 종료는 기존대로 중단
- **서브에이전트 리뷰 강제** — Claude 리뷰는 메인 에이전트가 아닌 Agent 툴 서브에이전트가 독립 수행 (이해충돌 방지). 결과는 별도 파일 보존 없이 PRD 본문 자체에 흡수
- **자동 머지 가드 3종 AND** — `rp-ship` 자동 머지는 (a) CI SUCCESS · (b) PR base 정상 · (c) `MERGEABLE` 모두 충족 시에만 진행. 하나라도 실패 시 OPEN 유지 + 사용자 보고
